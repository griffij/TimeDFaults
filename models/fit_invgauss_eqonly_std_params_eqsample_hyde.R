...
for (datalist in datalists){
    ...
    index = 1
    figure_filename = paste0('plots/invgauss_fit_eqonly_std_param_eqsample_hyde', j, '.pdf')
    pdf(figure_filename)
    ...
    bayes.mod.fit <- jags(data = sim.data.jags, inits = bayes.mod.inits,
		  ...
		  model.file = 'invgauss_eqonly_std_param_hyde.jags')
    print(bayes.mod.fit)
    ...
    filename = paste0('outputs/df_posterior_eqonly_', j, '_hyde.csv')
    print(filename)
...
png(file='plots/invgauss_fit_eqonly_std_param_hyde.png', units="in", width=5, height=5, res=300)
